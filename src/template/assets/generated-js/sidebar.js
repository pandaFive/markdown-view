...
function setActiveTocLink(activeId) {
    if (!appContext.sidebar.currentTocTracking)
        return;
    if (appContext.sidebar.currentActiveTocId === activeId)
        return;
...
function findTrackedHeading(id) {
    if (!appContext.sidebar.currentTocTracking || !id)
        return null;
...
function getPendingTocNavigationId(activationOffset) {
    if (!appContext.sidebar.pendingTocNavigationId)
        return '';
    ...
    if (navigationTop <= activationOffset + TOC_NAVIGATION_SLACK_PX &&
...
function scheduleTocTrackingUpdate() {
    if (!appContext.sidebar.currentTocTracking || appContext.sidebar.tocTrackingFrame !== null)
        return;
...
function ensureSuppressedTocTrackingResume() {
    ...
    appContext.sidebar.suppressTocTrackingTimer = window.setTimeout(function () {
        ...
        if (!appContext.sidebar.pendingSuppressedTocTrackingUpdate)
            return;
