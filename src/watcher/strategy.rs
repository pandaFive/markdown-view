...
pub(super) fn handle_debounced_events(
    strategy: &WatchStrategy,
    events: Vec<DebouncedEvent>,
) -> Vec<PathBuf> {
    strategy.collect_changed_paths(&events)
}
...
mod tests {
    ...
    use super::{
        handle_debounced_events, is_content_change_event, is_hidden_relative, is_target_file,
        is_within_base_dir, WatchStrategy,
    };
    ...
    fn test_handle_debounced_events_単一ファイル対象のみ通知する() {
        ...
        let received = handle_debounced_events(
            &strategy,
            vec![
