...
- [x] メモ書き込みを `tmp + rename` で原子化する
  ...
  - 2026-04-30 時点の旧実装: `MemoFs::write` が `tokio::fs::write`（内部 truncate+write）で、トレイトコメントも atomic write を要求していなかった。書き込み途中の電源断・クラッシュでメモが空 or 部分書き込みで破損する。また `ensure_safe_memo_path()` の symlink 検査と実 write の間に TOCTOU window が残っていた
