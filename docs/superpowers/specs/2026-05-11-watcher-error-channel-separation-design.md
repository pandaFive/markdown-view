...
- `error_tx/error_rx`: `WatchError` を流す異常通知専用 bounded channel。容量は `WATCHER_ERROR_MESSAGE_BUFFER` として明示し、送信は `blocking_send` を使う。通常変更通知とは別容量にし、`FileChanged` の満杯で破棄されない。
...
- `send_error_event(error_tx, error, label)`: error 専用 channel へ `blocking_send` する。file channel の満杯とは独立させる。receiver closed 時は panic せず warn/error log に残す。
...
- error channel full: `blocking_send` で内部 forwarder が drain するまで待つ。watcher runtime は標準スレッド上で動くため、Tokio runtime worker を塞がない。外部 receiver が閉じた場合は closed として扱う。
