...
今回の変更では、通常変更イベントの過負荷耐性は維持しつつ、異常通知だけを receiver が開いている限り送達する。
...
`WatchEvent::Error` は専用 helper に渡し、`mpsc::Sender::blocking_send` で送る。チャネルが満杯でも receiver が開いていれば、空きができるまで待って送達する。receiver が閉じている場合だけ `SendError` として warn ログに残し、送達不能を観測可能にする。
...
4. `blocking_send` で receiver へ送達する。
5. receiver closed の場合は warn し、送達不能だった事実をログに残す。

health latch を error event 送信前に維持するため、仮に receiver 側が詰まり `blocking_send` が待っていても、`Watcher::health()` では先に `Failed(_)` を観測できる。

## エラー処理

`WatchEvent::Error` の送信失敗として扱うのは、receiver が閉じている場合だけである。この場合、foreground が既に終了している、または watch event stream が消費されない状態なので、再送機構は追加しない。代わりに `label` と error detail を warn ログへ残す。
...
- `Error` は channel full 時でも receiver が開いていれば送達されることを確認する。
...
`Error` の channel full テストは、容量 1 の channel を `FileChanged` で埋めた状態で別スレッドから `send_watch_event(WatchEvent::Error(...))` を呼ぶ。メイン側が先に `FileChanged` を drain した後、`Error` を受信できることを確認する。これにより、`Error` が満杯時に破棄されない契約を再現可能にする。
...
- `WatchEvent::Error` は `mpsc` channel が満杯でも、receiver が開いている限り破棄されない。
...
`blocking_send` は異常系だけに限定する。通常の `FileChanged` burst で watcher thread が長時間詰まる経路は増やさない。

## ロールバック

`send_watch_event` の variant 分岐、`WatchEvent::Error` 用 blocking helper、追加・更新テストを revert すれば元に戻せる。

外部 API、WebSocket payload、UI は変更しないため、ロールバック時の利用者向け互換性リスクは低い。残リスクは、receiver が長時間詰まっている場合に error 送信中の watcher thread が待つことだが、failure health は送信前に latch され、silent failure 回避を優先する判断とする。
