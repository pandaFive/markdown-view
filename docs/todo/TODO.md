...
- [x] `panic::catch_unwind` の init 経路で `init_tx` 残存時に `ThreadPanic` を init 結果として送出
  - 完了根拠: watcher thread panic handler に `init_tx` を渡し、初期化結果送信前に panic した場合は `WatchError::thread_panic(...)` を `InitResult` の `Err` として返す構成にした。同じ panic detail は既存どおり `WatchEvent::Error` 補助通知にも流し、health は `Failed(ThreadPanic)` に latch する。`init_tx` が `None` の稼働後 panic と shutdown 中 panic は init result へ触れず、従来どおり health failed と error event で扱う。String payload と `anyhow::Error` payload の detail が init result と error event に保持されることを unit test で固定した。HTTP API、UI、WebSocket payload、shutdown API、watcher event shape は変更していない
