...
最終整理: 2026-06-06。High / Medium 候補を再評価し、RSS plateau 切り分けを Done Summary へ圧縮した。未完了の低優先候補は `BACKLOG.md` に残す。
...
- [x] ディレクトリ検索 many-match の逐次照合と prefix 読みで cold elapsed を抑制する
  ...
  - 残余リスク: 当時は RSS plateau が未解決で、prefix 後も検索後に release server RSS が約 1.0-1.1GiB まで上がり、`RssAnon` が支配的だった。`spawn_blocking` を専用小 stack thread に置き換える実験は効果がなかったため採用していない。安全境界なしの巨大単一 block は prefix fast path ではなく全文読みに戻るため、この形状の性能確認と RSS 原因切り分けを follow-up として残していたが、2026-06-06 の RSS plateau 切り分け完了判断で High / Medium の未完了候補からは外し、必要時の追加診断は `BACKLOG.md` P2 へ移した。
...
- [x] ディレクトリ検索の 10MiB 近傍 many-match 経路を早期停止・処理単位見直しで抑制する
  ...
  - 残余リスク: 上記の Done 判断は warm 経路の構造改善に限定する。dev build の cold run は初回だけ大きく、server RSS も warm run で完全な低 plateau とは断定しないため、当時は原因切り分けを follow-up として継続した。2026-06-06 の RSS plateau 切り分け完了判断で High / Medium の未完了候補からは外し、必要時の追加診断は `BACKLOG.md` P2 へ移した。
