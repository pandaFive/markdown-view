...
- [x] watcher の `try_send` で `WatchEvent::Error` を `FileChanged` と同列に破棄しない
  - 完了根拠: `send_watch_event` が `WatchEvent` variant ごとに送信方針を分け、`FileChanged` は従来どおり満杯時に drop する一方、`Error` は満杯時に watcher 単位の bounded helper へ代表エラーを委譲し、送達中の追加 `Error` は件数集約する構成になった。notify error、internal channel failure、新規監視登録失敗、init 完了後の watcher thread panic は既存の health failure latch を維持したまま error event として foreground へ流れる。満杯時の `FileChanged` drop、満杯時の `Error` 送達、送達中追加 `Error` の集約、別 watcher state 間の独立性、receiver closed 時の `Error` 非panic、current_thread runtime 上での満杯時送達を unit test で固定した。外部 HTTP API、UI、WebSocket error payload は変更していない。残リスクとして、送達中の追加 `Error` は全件 event としては届かず件数集約ログで観測するが、watcher thread の非停止性と unbounded queue 回避を優先する
