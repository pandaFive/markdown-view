...
- [x] watcher の `try_send` で `WatchEvent::Error` を `FileChanged` と同列に破棄しない
  - 完了根拠: `send_watch_event` が `WatchEvent` variant ごとに送信方針を分け、`FileChanged` は従来どおり満杯時に drop する一方、`Error` は満杯時に process-wide の専用送達 worker へ委譲し、receiver が開いている限り送達する構成になった。notify error、internal channel failure、新規監視登録失敗、thread panic は既存の health failure latch を維持したまま error event として foreground へ流れる。満杯時の `FileChanged` drop、満杯時の `Error` 送達、receiver closed 時の `Error` 非panic、current_thread runtime 上での満杯時送達を unit test で固定した。外部 HTTP API、UI、WebSocket error payload は変更していない。残リスクとして、receiver が開いたまま長時間 drain しない場合は専用 worker queue に ErrorDelivery が滞留し得るが、Error を silent drop しない要件とのトレードオフとして扱う
