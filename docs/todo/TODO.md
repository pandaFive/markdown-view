...
- [ ] ディレクトリ検索 many-match の RSS plateau を切り分ける
  - 現状: 2026-06-03 に block 単位の逐次照合、result-limit 到達後の同一ファイル tail 抽出停止、ASCII many-match prefix 読みを追加し、10MiB many-match 単一ファイルの release 検索は elapsed 0.0192s、`searched_bytes=10485760`、`truncated_reasons=["result_limit"]` まで改善した。一方で同じサーバープロセスは検索5秒後に RSS 1106676 KiB / `RssAnon=1099604 KiB` の anonymous plateau を示し、`spawn_blocking` を専用小 stack thread に置き換える実験でも改善しなかった。検索アルゴリズムは全文 byte validation を維持しつつ、安全な Markdown block/paragraph 境界が取れる many-match では全文 parse/search を避ける。安全境界なしの巨大単一 block は correctness 優先で全文読みに戻すため、残課題は Tokio runtime / glibc allocator / 環境依存の初回リクエスト後 anonymous map と安全境界なしケースの性能切り分けに絞る。
  - 受け入れ条件: release build と dev build、cold/warm、単一巨大ファイル/複数ファイルを分けた測定を行い、結果を実パスや full process args なしで記録する。RSS plateau については `/proc/*/status` と map 概要を使い、Tokio worker、blocking pool、allocator arena、WSL2 計測特性のどれが支配的かを特定する。
