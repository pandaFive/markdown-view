...
- [ ] ディレクトリ検索 many-match の cold run と RSS plateau を切り分ける
  - 現状: 10MiB 近傍 many-match 経路は warm run で改善したが、dev build の初回 cold run は elapsed 2:54.45 と大きく、server RSS も完全な低 plateau とは断定できない。検索結果上限と stale cancellation の処理量抑制とは別に、cold cache / debug build / Markdown parsing / allocation の寄与を分離して測定する。
