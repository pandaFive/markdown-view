...
- [x] ディレクトリ検索 many-match の逐次照合と prefix 読みで cold elapsed を抑制する
  ...
  - 残余リスク: RSS plateau は未解決。prefix 後も検索後に release server RSS が約 1.0-1.1GiB まで上がり、`RssAnon` が支配的だった。`spawn_blocking` を専用小 stack thread に置き換える実験は効果がなかったため採用していない。RSS 原因切り分けは Medium Priority の follow-up として継続する。
