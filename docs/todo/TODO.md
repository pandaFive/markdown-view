...
- [ ] ディレクトリ検索 many-match の RSS plateau を切り分ける
  - 現状: 2026-06-03 に block 単位の逐次照合、result-limit 到達後の同一ファイル tail 抽出停止、ASCII many-match prefix 読みを追加し、10MiB many-match 単一ファイルの release 検索は elapsed 0.0017s、`searched_bytes=4545`、`truncated_reasons=["result_limit"]` まで改善した。一方で同じサーバープロセスは検索5秒後に RSS 1011988 KiB / `RssAnon=1004924 KiB` の anonymous plateau を示し、`spawn_blocking` を専用小 stack thread に置き換える実験でも改善しなかった。検索アルゴリズムの全文 scan は避けられているため、残課題は Tokio runtime / glibc allocator / 環境依存の初回リクエスト後 anonymous map の切り分けに絞る。
...
- [x] ディレクトリ検索 many-match の逐次照合と prefix 読みで cold elapsed を抑制する
  - 完了根拠: `search_directory_with_limits_blocking()` がファイル全体の block 抽出結果を保持せず、`search_file_streaming_blocks()` へ残り result 予算を渡して block ごとに照合する構成にした。前後 context の互換性は 1 block lookahead で維持し、result-limit 到達時は後続 block 抽出を停止する。巨大 block は既存 direct search 経路を使い、stale cancellation は block 抽出・正規化・巨大 block search に伝播する。さらに ASCII query で raw hit が result 予算 + 1 件見つかる many-match では、段落境界までの prefix だけを Markdown parser に渡し、prefix で result-limit に到達しなければ全文読みに fallback するため、除外対象 Markdown による false positive でも検索結果の正しさを維持する。SearchResponse JSON の `truncated_reasons` は既存の `result_limit` / `file_limit` / `byte_limit` だけを使い、新しい truncation reason は追加していない。Host/Origin 検証、path validation、HTML sanitize、CSP、ファイルサイズ上限は変更していない。計測 fixture は `/tmp/markdown-view-search-many-match-streaming.***` に生成し、repo へ追加していない。
  - 計測: 10MiB many-match 単一ファイルの release 再測定は `results=100`, `searched_files=1`, `searched_bytes=4545`, `truncated=true`, `truncated_reasons=["result_limit"]`, elapsed 0.0017s。実装途中の prefix 前 release 単一計測は elapsed 0.03-0.04s、`searched_bytes=10485760` だったため、cold elapsed と scan byte は改善した。複数ファイルケースは既存 result-limit 停止で `searched_files=2`, `searched_bytes=8100`, elapsed 0.00s 台を維持した。
