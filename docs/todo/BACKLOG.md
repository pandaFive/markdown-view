...
- [ ] ディレクトリ検索 prefix many-match RSS plateau の環境差・live allocation 追加診断を必要時に行う
  ...
  - 追加診断: 2026-06-09 に `scripts/measure-search-rss-plateau.mjs --timeline --strict --modes release --fixtures prefix --runs cold --fixture-scale full --fixture-density dense --settled-delays 1s,5s --allocator-profiles default,arena1` を実行した。acceptanceStatus=`full`, fullAcceptanceMet=`true`。raw response body、実パス、full process args、raw `/proc/maps` 行、親環境変数値は記録していない。
